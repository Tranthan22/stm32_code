...
//  uint16_t data_send_len=0;
  ...
  if( memcmp(TCP_Struct_Frame->DestIP,ip,4) )return;
  ...
  //Received a connecting request from client
  ...
  {
	  //after server confirmed
	  //client do something here
  }

  //connection closed
  else if(TCP_Struct_Frame->TCP_Flags == (TCP_FIN|TCP_ACK)){
	//close request from server
	...
	NET_SendFrame((uint8_t *)TCP_Struct_Frame,len);
	  ...
	  //received data restored in "TCP_Struct_Frame->data"
	  // do something here
...
uint8_t TCP_Connect(uint8_t *IP_server,uint16_t port,uint32_t timeout,int8_t Try_reconnect){
	...
	do{
		if(TCP_sendSYN(IP_server,port,timeout)){
		// gửi bản tin SYN
			UART_putString("Ket noi that bai \r\n");
			return 0; //gui that bai
		}
		else{
			t = HAL_GetTick();
			while(1){
				 ...
				 if(HAL_GetTick() - t > timeout) break;
			}
		}
	}while(Try_reconnect--);
